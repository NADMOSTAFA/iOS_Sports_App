...
extension LeagueDetailsViewController : UICollectionViewDelegate {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if indexPath.section == 2 {
            ...
            if let TeamDetailVC  = storyboard?.instantiateViewController(withIdentifier: "TeamDetails") as? TeamDetailsViewController{
                
                let index = Array(teamList!.keys)[indexPath.row]
                leagueDetailsViewModel.setselectedTeam(team: self.teamList![index]!)
                TeamDetailVC.leagueDetailsViewModel = leagueDetailsViewModel
                print(teamList![index]?.teamName)
